...
# classifier
netC = dcganv2._netC(opt.imageSize, nz, nc-43, ndf, ngpu, n_extra_layers)
...
for epoch in range(opt.niter):
    ...
    while i < len(dataloader):
        ...
        # (2) Update G network
        ###########################
#         label.resize_as_(labels_range).copy_(labels_range)
...
#         labelv = Variable(tmp.float())
        ...
        cat_fake = netC(Variable(fake.data))
        

        cat_real = netC(Variable(input))
        cat_loss  = criterion(cat_real, labelv) + criterion(cat_fake, labelv)
        
        #print (cat_loss.data.cpu().numpy()[0])
        ...
        errG =1/2*torch.mean(fake_labels) + 1/2*criterion(netC(fake), labelv)
        print (errG)
        print (torch.mean(fake_labels) )
