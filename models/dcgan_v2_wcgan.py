...
class _netG(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(     nz+43 , ngf * 8, 4, 1, 0, bias=False),
    ...
    def forward(self, input, label):
        ...
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            input  = torch.cat([input,label],1).view(-1,self.nz + 43, 1, 1)
...
class _netD(nn.Module):
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0):
        ...
        self.main.add_module('final_layers.conv', nn.Conv2d(ndf * 8, 1, 4, 1, 0, bias=False))
        
        #self.main.add_module('final_layers.sigmoid', nn.Sigmoid())
    ...
    def forward(self, input, label):
        ...
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            y_onehot = label.cuda()
            input_2 = torch.cat([input, label], 1)
            #print (input_2.size())
            output = self.main(input_2)
...
class _netC(nn.Module):
    ...
    def forward(self, input, label):
        ...
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            output = self.main(torch.cat([input, label], 1))
            
            #output = output.view(-1, 43)
            #sig = nn.Softmax()
            #output = sig(output)
            #print (output.size())
        return output#.view(-1, 43)
